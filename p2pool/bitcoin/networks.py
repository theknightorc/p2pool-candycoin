...
nets = dict(
    ...
    candycoin=math.Object(
        P2P_PREFIX='f8a9b4f0'.decode('hex'),
        P2P_PORT=6152,
        ADDRESS_VERSION=78,
        RPC_PORT=6157,
        RPC_CHECK=defer.inlineCallbacks(lambda bitcoind: defer.returnValue(
        ...
        BLOCK_PERIOD=60, # s
        SYMBOL='YUM',
        CONF_FILE_FUNC=lambda: os.path.join(os.path.join(os.environ['APPDATA'], 'Candycoin') if platform.system() == 'Windows' else os.path.expanduser('~/Library/Application Support/Candycoin/') if platform.system() == 'Darwin' else os.path.expanduser('~/.candycoin'), 'candycoin.conf'),
        BLOCK_EXPLORER_URL_PREFIX='http://explorer.candyco.in:2750/block/',
        ADDRESS_EXPLORER_URL_PREFIX='http://explorer.candyco.in:2750/address/',
        TX_EXPLORER_URL_PREFIX='http://explorer.candyco.in:2750/tx/',
        SANE_TARGET_RANGE=(2**256//1000000000 - 1, 2**256//1000 - 1),
        DUMB_SCRYPT_DIFF=2**16,
        DUST_THRESHOLD=0.03e8,
    ),
